...
export const buildVoucherHtml = (data, branding, agentBranding = {}) => {
  ...
  const helplinePhone = "+91-8851346665";
  ...
  const hotelsHtml = displayHotels.length > 0 ? displayHotels.map((h) => {
    const hTitle = h.title || h.hotelName || h.name || (destinationVal ? `${destinationVal} Hotel` : "Hotel Accommodation");
    ...
    const hCheckInObj = h.checkIn ? new Date(h.checkIn) : (h.startDate ? new Date(h.startDate) : startObj);
    ...
    const hNights = h.nights || h.numberOfNights || nights || 1;
    const hCheckOutObj = h.checkOut ? new Date(h.checkOut) : (h.endDate ? new Date(h.endDate) : (hCheckInObj && !isNaN(hCheckInObj.getTime()) ? new Date(hCheckInObj.getTime() + hNights * 86400000) : endObj));
    ...
    const roomTypeStr = `${h.numberOfRooms || h.rooms || 1} x ${h.roomType || h.roomCategory || "Superior King Room"}`;
    const paxDetailStr = h.pax || paxVal || "2 Pax";
              ...
              <table style="width: 100%; border-collapse: collapse; border: 1px solid #b3cae8;">
                ...
                <tbody>
                  <tr>
                    ...
                    <td style="padding: 10px 12px; background-color: #ffffff; border: 1px solid #b3cae8; font-size: 12px; color: #000000; vertical-align: top;">
                      <div style="font-weight: 500; color: #000000;">${roomTypeStr}</div>
