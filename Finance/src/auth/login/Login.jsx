...
const roleRedirectMap = {
  finance_partner: {
    message: "Welcome Finance Partner",
...
const Login = () => {
  ...
  const { loading, error, token, justLoggedIn, user } = useSelector(
    (state) => state.auth,
  );
  ...
  const validateLoginForm = (values) => {
    ...
    if (!values.password) nextErrors.password = "Password required";
      ...
      nextErrors.password = "Min 5 characters";
    return nextErrors;
  };
  ...
  const validateRecoveryStep = (step, values) => {
    ...
    if (step === "reset") {
      if (!values.newPassword) nextErrors.newPassword = "Password required";
        ...
        nextErrors.newPassword = "Min 6 characters";
  ...
  const handleRoleSelect = (roleValue) => {
    ...
    const nextErrors = validateLoginForm({ ...form, role: roleValue });
  ...
  const handleRecoveryChange = (field, value) => {
    ...
    setRecoveryErrors((prev) => {
      if (!prev[field]) return prev;
      const nextErrors = validateRecoveryStep(forgotStep, {
        ...recovery,
        [field]: nextValue,
      });
      return { ...prev, [field]: nextErrors[field] };
  ...
  useEffect(() => {
    if (token && justLoggedIn && user) {
      if (!allowedWorkspaceRoles.has(user.role)) {
        toast.error("This login form is only for Finance users.");
        dispatch(logout());
  ...
  }, [token, justLoggedIn, user, navigate, dispatch, form.role]);
  ...
  useEffect(() => {
    ...
    if (node) {
      ...
      const nextMaxHeight = Math.min(
        240,
        Math.max(120, Math.floor(available - 12)),
      );
          ...
          <AnimatePresence mode="wait" initial={false}>
            {authView === "login" ? (
              ...
              >
                ...
                <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 text-center sm:text-left">
                  Welcome back
                </h2>
                <p className="mb-4 mt-1.5 text-xs sm:text-sm text-gray-500 text-center sm:text-left">
                  Enter your Finance credentials to access your dashboard
                </p>

                <form className="space-y-5" onSubmit={handleSubmit}>
                  <div className="relative z-10" ref={roleMenuRef}>
                    <div className="mb-2 flex items-center justify-between gap-3">
                      <label className="text-sm sm:text-md font-semibold text-slate-800">
                        Access Role <span className="text-red-700">*</span>
                      </label>
                      <ValidationPill message={errors.role} />
                    </div>

                    {(() => {
                      const activeRole =
                        roles.find((r) => r.value === form.role) || null;
                      ...
                      return (
                        <>
                          <button
                            ...
                            className={`flex w-full items-center justify-between gap-3 rounded-2xl border px-4 py-2 sm:py-2.5 text-sm sm:text-base outline-none transition-all ${
                              errors.role
                                ? "border-rose-300 bg-rose-50/40 text-rose-900"
                                : "border-gray-300 bg-white hover:border-slate-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                            }`}
                          >
                                  ...
                                  {roles.map((r) => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`flex w-full items-center gap-2.5 px-3 py-2 text-left text-sm sm:text-base transition ${
                                          selected
                                            ? "bg-slate-50"
                                            : "hover:bg-slate-50"
                                        }`}
                  ...
                  <div className="relative">
                    ...
                    >
                      {showPassword ? (
                        <Eye className="h-5 w-5 stroke-[1.8] text-gray-500 hover:text-gray-700" />
                      ) : (
                        <EyeOff className="h-5 w-5 stroke-[1.8] text-gray-500 hover:text-gray-700" />
                      )}
                    </span>
                  </div>
              ...
              >
                <div className="mb-5 flex items-center justify-between gap-4">
                  <button
                    type="button"
                    onClick={() =>
                      resetForgotFlow(recovery.email || form.email)
                    }
                ...
                <p className="mb-4 mt-1.5 text-xs sm:text-sm text-gray-500">
                  Recover your account in three quick steps.
                </p>
                ...
                {forgotStep !== "success" ? (
                  <div className="mb-6 flex flex-wrap gap-2">
                    <StepPill
                      index={1}
                      label="Email"
                      active={recoveryStepIndex === 1}
                      complete={recoveryStepIndex > 1}
                    />
                    <StepPill
                      index={2}
                      label="OTP"
                      active={recoveryStepIndex === 2}
                      complete={recoveryStepIndex > 2}
                    />
                    <StepPill
                      index={3}
                      label="Password"
                      active={recoveryStepIndex === 3}
                      complete={false}
                    />
                  </div>
                ) : null}
                  ...
                  <div className="space-y-5">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleRecoveryChange("email", e.target.value)
                        }
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        value={recovery.otp}
                        onChange={(e) =>
                          handleRecoveryChange("otp", e.target.value)
                        }
                  ...
                  <div className="space-y-5">
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleRecoveryChange("newPassword", e.target.value)
                        }
                    ...
                    <div className="relative">
                      <div className="mb-2 flex items-center justify-between gap-3">
                        ...
                        <ValidationPill
                          message={recoveryErrors.confirmPassword}
                        />
                      </div>
                      <input
                ...
                {forgotStep === "success" ? (
                  <div className="rounded-3xl border border-emerald-200 bg-[linear-gradient(180deg,#ffffff_0%,#f4fbf7_100%)] px-6 py-7 shadow-sm">
                    ...
                    <p className="mt-3 text-sm leading-7 text-slate-500">
                      Your account is ready. Return to sign in using the new
                      password.
                    </p>
                    <button
                      type="button"
