            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                paginatedPackages.map((pkg, idx) => {
                  ...
                  const pkgCreatorId = String(pkg.createdBy?._id || pkg.createdBy || pkg.supplier?._id || pkg.supplier || "");
                  const currentUserId = String(currentUser?._id || currentUser?.id || "");
                  const isCreatedByMe = Boolean(pkgCreatorId && currentUserId && pkgCreatorId === currentUserId);
                  const pkgCreatorRole = String(pkg.creatorRole || pkg.createdBy?.role || pkg.supplier?.role || "").toLowerCase().trim();
                  const userRole = String(currentUser?.role || "").toLowerCase().trim();
                  const isManagerOrAdmin = ["operation_manager", "ops_manager", "admin", "super_admin"].includes(userRole);
                  const canDelete = isCreatedByMe || isManagerOrAdmin;
                      ...
                      <td className="px-5 py-4 align-middle">
                        <div className="leading-tight">
                          <div className="flex items-center gap-1.5 mb-0.5">
                            ...
                            ) : pkgCreatorRole === "operation_manager" || pkgCreatorRole === "ops_manager" || pkgCreatorRole === "manager" ? (
                              <span className="rounded bg-purple-50 border border-purple-200 px-1.5 py-0.2 text-[9px] font-bold text-purple-700">
                                Manager Template
                              </span>
                            ) : pkgCreatorRole === "admin" || pkgCreatorRole === "super_admin" ? (
                              <span className="rounded bg-amber-50 border border-amber-200 px-1.5 py-0.2 text-[9px] font-bold text-amber-700">
                                Admin Template
                              </span>
                            ) : pkg.creatorName ? (
                              <span className="rounded bg-slate-100 border border-slate-200 px-1.5 py-0.2 text-[9px] font-bold text-slate-600">
                                {pkg.creatorName}
                              </span>
                            ) : null}
                          </div>
