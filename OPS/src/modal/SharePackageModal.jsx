...
export default function SharePackageModal({
  ...
  useEffect(() => {
    ...
    if (shareMode === "VOUCHER") {
      ...
      setEmailPreviewError("");
      try {
        const clientNameVal = query?.name || query?.clientName || query?.customerName || query?.guestName || "Valued Client";
        const companyNameVal = effectiveUser?.brandingName || effectiveUser?.companyName || effectiveUser?.name || "DDLC Company Pvt. Ltd.";
        const voucherNo = query?.voucherNumber || `VCH-${query?.queryId || "001"}`;
        const destinationVal = query?.destination || "Destination";
        const startDateVal = query?.startDate ? new Date(query.startDate).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }) : "-";
        const endDateVal = query?.endDate ? new Date(query.endDate).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }) : "-";
        const datesVal = startDateVal !== "-" && endDateVal !== "-" ? `${startDateVal} to ${endDateVal}` : startDateVal;
        
        // Calculate Duration
        const startObj = query?.startDate ? new Date(query.startDate) : null;
        const endObj = query?.endDate ? new Date(query.endDate) : null;
        let calcNights = 0;
        if (startObj && endObj && !isNaN(startObj.getTime()) && !isNaN(endObj.getTime())) {
          const diffTime = Math.abs(endObj - startObj);
          calcNights = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        }
        const nights = query?.nights || quote?.nights || calcNights || 4;
        const days = query?.days || quote?.days || (nights > 0 ? nights + 1 : 5);
        const durationVal = query?.duration || quote?.duration || `${nights}N / ${days}D`;

        const paxVal = `${(query?.numberOfAdults || 1)} Adults${query?.numberOfChildren > 0 ? `, ${query.numberOfChildren} Children` : ""}`;
        const rawServices = Array.isArray(quote?.services) && quote.services.length > 0 ? quote.services : [
          { type: "hotel", title: "Jaypee Residency Manor Mussoorie", confirmation: "Confirmed(Confirmed)" },
          { type: "hotel", title: "The Landour Retreat Mussoorie", confirmation: "Confirmed(Confirmed)" },
        ...
        const companyAddressVal = effectiveUser?.address || effectiveUser?.companyAddress || "KG 3/69, Ground Floor, Vikas Puri, New Delhi,, Near UK Nursing Home, New Delhi, Delhi, India - 110018";
        const companyPhoneVal = effectiveUser?.phone || "+91-8851346665";
        const companyEmailVal = effectiveUser?.email || "varun@holidaycircuit.com";
        // Keep the voucher aligned with the logo shown in the agent workspace
        // navigation. A quote may retain an older logo, so it must not override
        // the agent's current branding logo.
        ...
        const serviceRowsHtml = rawServices.map((s, idx) => {
          const typeStr = (s.type || "Service").toUpperCase();
          const titleStr = s.title || s.name || "Service details";
          const rawConf = String(s.confirmation || s.status || "Confirmed").trim();
          
          // Exact DB confirmation number without fake fallbacks
          ...
          const isPending = rawConf.toLowerCase().includes("pending") || cnfNumDisplay === "-";
          // Keep the status as plain text so email and PDF show the same value.
          const statusLabel = isPending ? "Pending" : "Confirmed";
          const rowBg = idx % 2 === 1 ? "background-color: #f1f5f9;" : "background-color: #f8fafc;";

          return `
            <tr style="border-bottom: 1px solid #e2e8f0; ${rowBg}">
              <td style="width: 13%; padding: 10px 14px; font-weight: 700; color: #475569; font-size: 11px; text-transform: uppercase; border-radius: 0px; border-right: 1px solid #e2e8f0;">${typeStr}</td>
              <td style="width: 47%; padding: 10px 14px; font-weight: 600; color: #0f172a; font-size: 13px; line-height: 1.5; overflow-wrap: anywhere; border-radius: 0px; border-right: 1px solid #e2e8f0;">${titleStr}</td>
              <td style="width: 20%; padding: 10px 14px; font-weight: 600; color: #0f172a; font-size: 12px; border-radius: 0px; border-right: 1px solid #e2e8f0;">
                ${statusLabel}
              </td>
              <td style="width: 20%; padding: 10px 14px; font-weight: 700; color: #0f172a; font-size: 12px; border-radius: 0px; font-family: monospace; overflow-wrap: anywhere;">
                ${cnfNumDisplay}
              </td>
            </tr>
          `;
        ...
        const emailVoucherHtml = `
          <div style="font-family: 'Plus Jakarta Sans', Arial, sans-serif; color: #1e293b; width: 100%; padding: 0px; box-sizing: border-box; background: #ffffff;">
            ...
            <!-- AGENT BRAND HEADER BANNER (MATCHING REFERENCE SCREENSHOT) -->
            ...
            <!-- TRAVEL VOUCHER OVERVIEW TABLE (LIGHT NAVY BLUE HEADER TINT & DURATION INCLUDED) -->
            <div style="margin-bottom: 20px; padding: 0 20px;">
              <table style="width: 100%; border-collapse: collapse; font-size: 13px; border: 1px solid #cbd5e1; border-radius: 0px;">
                <thead>
                  <tr style="background-color: #ebf5ff; border-bottom: 2px solid #bfdbfe; text-align: left;">
                    <th colspan="2" style="padding: 10px 14px; font-size: 12px; font-weight: 800; color: #1e3a8a; text-transform: uppercase; letter-spacing: 0.6px; border-radius: 0px;">
                ...
                <tbody>
                  <tr style="border-bottom: 1px solid #e2e8f0;">
                    <td style="padding: 9px 14px; color: #475569; width: 160px; font-weight: 600; background-color: #f1f5f9; border-right: 1px solid #e2e8f0;">Voucher Number:</td>
                    <td style="padding: 9px 14px; color: #0f172a; font-weight: 700; background-color: #f8fafc;">${voucherNo}</td>
                  </tr>
                  <tr style="border-bottom: 1px solid #e2e8f0;">
                    <td style="padding: 9px 14px; color: #475569; font-weight: 600; background-color: #f1f5f9; border-right: 1px solid #e2e8f0;">Guest Details:</td>
                    <td style="padding: 9px 14px; color: #0f172a; font-weight: 700; background-color: #f8fafc;">${clientNameVal}</td>
                  </tr>
                  <tr style="border-bottom: 1px solid #e2e8f0;">
                    <td style="padding: 9px 14px; color: #475569; font-weight: 600; background-color: #f1f5f9; border-right: 1px solid #e2e8f0;">Destination:</td>
                    <td style="padding: 9px 14px; color: #0f172a; font-weight: 700; background-color: #f8fafc;">${destinationVal}</td>
                  </tr>
                  <tr style="border-bottom: 1px solid #e2e8f0;">
                    <td style="padding: 9px 14px; color: #475569; font-weight: 600; background-color: #f1f5f9; border-right: 1px solid #e2e8f0;">Duration:</td>
                    <td style="padding: 9px 14px; color: #0f172a; font-weight: 700; background-color: #f8fafc;">${durationVal}</td>
                  </tr>
                  <tr style="border-bottom: 1px solid #e2e8f0;">
                    <td style="padding: 9px 14px; color: #475569; font-weight: 600; background-color: #f1f5f9; border-right: 1px solid #e2e8f0;">Travel Dates:</td>
                    <td style="padding: 9px 14px; color: #0f172a; font-weight: 600; background-color: #f8fafc;">${datesVal}</td>
                  </tr>
                  <tr>
                    <td style="padding: 9px 14px; color: #475569; font-weight: 600; background-color: #f1f5f9; border-right: 1px solid #e2e8f0;">Passengers:</td>
                    <td style="padding: 9px 14px; color: #0f172a; font-weight: 600; background-color: #f8fafc;">${paxVal}</td>
                  </tr>
                </tbody>
              </table>
            </div>

            <!-- SERVICE DETAILS TABLE (LIGHT NAVY BLUE HEADER TINT & 4 COLUMNS) -->
            <div style="margin-bottom: 24px; padding: 0 20px;">
              <table style="width: 100%; table-layout: fixed; border-collapse: collapse; font-size: 12px; border: 1px solid #cbd5e1; border-radius: 0px;">
                <thead>
                  <tr style="background-color: #ebf5ff; text-align: left; border-bottom: 2px solid #bfdbfe;">
                    <th style="width: 13%; padding: 10px 14px; font-weight: 800; color: #1e3a8a; font-size: 11px; text-transform: uppercase; border-radius: 0px; border-right: 1px solid #bfdbfe;">TYPE</th>
                    <th style="width: 47%; padding: 10px 14px; font-weight: 800; color: #1e3a8a; font-size: 11px; text-transform: uppercase; border-radius: 0px; border-right: 1px solid #bfdbfe;">SERVICE DESCRIPTION</th>
                    <th style="width: 20%; padding: 10px 14px; font-weight: 800; color: #1e3a8a; font-size: 11px; text-transform: uppercase; border-radius: 0px; border-right: 1px solid #bfdbfe;">CONFIRMATION STATUS</th>
                    <th style="width: 20%; padding: 10px 14px; font-weight: 800; color: #1e3a8a; font-size: 11px; text-transform: uppercase; border-radius: 0px;">CONFIRMATION NUMBER</th>
                  </tr>
                </thead>
                <tbody>
                  ${serviceRowsHtml}
