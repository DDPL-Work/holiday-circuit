...
async function run() {
  try {
    ...
    const currentUserId = accessContext.currentUserId;
    const baseScopedInternalInvoices = allInternalInvoices.filter((invoice) => {
      ...
      return assignedFinanceId === currentUserId;
    });
    ...
    const scopedInternalInvoices = candidateScopedInternalInvoices
      .filter((invoice) => {
        ...
        return Array.from(internalKeys).some((key) => scopedInvoiceQueryKeys.has(key));
      });

    const start = new Date(2026, 6, 1);
    const end = new Date(2026, 6, 31, 23, 59, 59, 999);

    const outwardTotal = sumInternalInvoiceAmountsInWindow(scopedInternalInvoices, start, end);
    console.log('July 2026 Outward Total (Simulated):', outwardTotal);
