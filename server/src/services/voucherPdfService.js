...
const drawTermsAndConditionsTable = (doc, pageCtx, y, terms = []) => {
  const rawTerms = Array.isArray(terms)
    ? terms
        .map((t) => (typeof t === "object" && t !== null ? t.text || t.content || JSON.stringify(t) : String(t || "").trim()))
        .filter((t) => t.length > 0)
    : typeof terms === "string"
...
export const generateVoucherPdf = async (voucherDetails) => {
  ...
  y = drawOverviewTable(doc, pageCtx, y, {
    ...
    issuedBy: normalizeCompanyName(
      voucherDetails.issuedBy || voucherDetails.branding?.name || voucherDetails.companyName || BRAND.name,
      BRAND.name
    ),
  });
  ...
  if (typeof termsList === "string") {
    termsList = [termsList];
  } else if (Array.isArray(termsList)) {
    termsList = termsList
      .map((t) => (typeof t === "object" && t !== null ? t.text || t.content || JSON.stringify(t) : String(t || "").trim()))
      .filter(Boolean);
  }
  ...
  y = drawHelplineTable(doc, pageCtx, y, {
    companyName: voucherDetails.branding?.name || voucherDetails.companyName || voucherDetails.brandingName || BRAND.name,
    phone: voucherDetails.branding?.phone || voucherDetails.companyPhone || voucherDetails.phone || BRAND.phone,
  });
