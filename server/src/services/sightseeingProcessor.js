...
export const processSightseeingExcel = async (filePath, ownerId) => {
  ...
  for (const row of normalizedRows) {
    ...
    const rowServiceName = String(row["Service Name"] || row["Sightseeing Name"] || "").trim();
    if (rowServiceName) {
    ...
    const description = String(row["Description"] || row["Service Description"] || "").trim();
  ...
  if (invalidRows.length > 0) {
    throw new Error(`Missing required service name or country in ${invalidRows.length} sightseeing services`);
  }
