...
const EditTerm = () => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await API.put(`/agent/terms/${id}`, { name, content });
      navigate(`/agent/terms/${id}`);
    } catch (error) {
  ...
  return (
    <section className="space-y-5 p-6 font-sans">
      <div className="flex items-center gap-2 text-sm text-gray-500 font-medium">
        ...
        <button onClick={() => navigate(`/agent/terms/${id}`)} className="hover:text-gray-900 transition-colors cursor-pointer truncate max-w-[160px]">
          {name || 'Details'}
        </button>
        <ChevronRight size={14} className="text-gray-400" />
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 space-y-6">
        ...
        <div className="border-t border-slate-200 pt-6 flex items-center gap-3">
          ...
          <button
            onClick={() => navigate(`/agent/terms/${id}`)}
