...
export const BasicDetailsTab = ({
  ...
  description,
  setDescription,
  isOpenedFromQuotationBuilder = false,
  partnerType,
}) => {
  return (
        ...
        <div>
          <p className="text-xs sm:text-sm text-slate-900 font-bold flex items-center gap-1.5">
            {isOpenedFromQuotationBuilder ? (
              <FileText size={15} className="text-[#3E63DD]" />
            ) : (
              <Package size={15} className="text-[#3E63DD]" />
            )}
            {isOpenedFromQuotationBuilder ? "1. Basic Quotation Information" : "1. Basic Package Information"}
          </p>
          <p className="text-xs text-gray-500 mt-0.5">
            {isOpenedFromQuotationBuilder
              ? "Configure destination, duration, and quotation details."
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 pt-1">
        <div className="sm:col-span-2">
          <label className="block text-xs font-semibold text-slate-700 mb-1">
            {isOpenedFromQuotationBuilder ? "Quotation Title" : "Package Title"} <span className="text-rose-500">*</span>
          </label>
          <input
            type="text"
            required
            placeholder={
              isOpenedFromQuotationBuilder
                ? "e.g. 4N/5D Mahabaleshwar Getaway Quotation"
                : "e.g. 5N/6D Mussoorie & Dhanaulti Hills Delight"
            }
            value={title}
        ...
        <div className="sm:col-span-2">
          <label className="block text-xs font-semibold text-slate-700 mb-1">
            {isOpenedFromQuotationBuilder ? "Quotation Description / Overview" : "Package Description / Overview"}
          </label>
          <textarea
            rows="3"
            placeholder={
              isOpenedFromQuotationBuilder
                ? "Short overview of the quotation, experience highlights, and key destination appeal..."
                : "Short overview of the holiday package, experience highlights, and key destination appeal..."
            }
            value={description}
