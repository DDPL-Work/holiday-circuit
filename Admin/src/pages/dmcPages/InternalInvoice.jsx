...
export default function InternalInvoice({ selectedQuery, queryServices = [] }) {
  ...
  const summary = useMemo(() => {
    ...
    const tcsAmount = Math.round((subtotal * Number(taxConfig.tcsRate || 0)) / 100);
  ...
  const handleGenerateInvoice = async (bypassConfirm = false) => {
    ...
    if (invoiceSource === "uploaded_invoice") {
      ...
      const subtotalMismatch =
        Math.abs(Math.round(Number(claimedSummary.subtotal || 0)) - Math.round(Number(summary.subtotal || 0))) > 1;
      const taxMismatch =
        Math.abs(Math.round(Number(claimedSummary.taxAmount || 0)) - Math.round(Number(summary.totalTax || 0))) > 1;
      const totalMismatch =
        Math.abs(Math.round(Number(claimedSummary.grandTotal || 0)) - Math.round(Number(summary.grandTotal || 0))) > 1;
              ...
              <div className="shrink-0 w-full lg:w-72">
                {isFileUploading ? (
                  ...
                  <div className="relative flex min-h-[86px] flex-col items-center justify-center rounded-md border border-emerald-200 bg-emerald-50/80 p-3.5 text-center shadow-2xs group hover:border-emerald-300 transition-all duration-300">
                    ...
                    <CheckCircle2
                      size={22}
                      className="mb-1 text-emerald-600 animate-scale-in"
                    />
                  ...
                  <label className="flex min-h-[86px] cursor-pointer flex-col items-center justify-center rounded-md border border-dashed border-slate-300 bg-white px-4 py-3 text-center text-sm font-semibold text-slate-700 shadow-2xs transition hover:border-blue-400 hover:bg-blue-50/30 group">
                    <Upload
                      size={18}
                      className="mb-1 text-blue-500 transition-transform group-hover:-translate-y-0.5 duration-200"
                    />
                ...
                >
                  <input
                    ...
                    value={
                      ...
                      claimedSummary.subtotal !== undefined &&
                      claimedSummary.subtotal !== null
                        ? Math.round(Number(claimedSummary.subtotal))
                        : ""
                    }
                    onChange={(event) => {
                ...
                >
                  <input
                    ...
                    value={
                      ...
                      claimedSummary.taxAmount !== undefined &&
                      claimedSummary.taxAmount !== null
                        ? Math.round(Number(claimedSummary.taxAmount))
                        : ""
                    }
                    onChange={(event) => {
                      const val = event.target.value;
                ...
                >
                  <input
                    ...
                    value={
                      ...
                      claimedSummary.grandTotal !== undefined &&
                      claimedSummary.grandTotal !== null
                        ? Math.round(Number(claimedSummary.grandTotal))
                        : ""
                    }
                    onChange={(event) => {
                      const val = event.target.value;
