...
const ServiceFieldLabel = ({ icon, label, iconClassName = "text-slate-400" }) => (
      ...
      queryServices
        ...
        .sort((left, right) => {
          const rankDifference =
            getServiceTypeSortRank(left.type) - getServiceTypeSortRank(right.type);
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-blue-50 text-blue-600">
                                {getServiceTypeIcon(service.type, "h-4 w-4")}
                              </span>
                            ...
                            <p className="mt-1 inline-flex items-center gap-1.5 text-[10px] text-slate-500">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {isServiceHidden ? <Eye size={15} /> : <EyeOff size={15} />}
                            </button>
                          </div>
                        </div>
                                  ...
                                  >
                                    ...
                                    {isHotelService ? (
                                      ...
                                      <div className="rounded-xl border border-slate-200 border-l-4 border-l-amber-500 bg-gradient-to-br from-amber-50/80 via-white to-amber-50/20 px-3 py-2.5 shadow-sm transition-all duration-500 ease-in-out hover:shadow-md hover:scale-[1.002]">
                                        ...
                                        <p className="mt-1 text-[13px] font-semibold text-slate-900">
                                          {formatServiceDate(
                                            service.serviceEndDate || service.resolvedServiceEndDate,
                                          )}
                                        </p>
                                      </div>
                                    )}
                                    ...
                                    <div className="rounded-xl border border-slate-200 border-l-4 border-l-teal-500 bg-gradient-to-br from-teal-50/80 via-white to-teal-50/20 px-3 py-2.5 shadow-sm transition-all duration-500 ease-in-out hover:shadow-md hover:scale-[1.002]">
                                      ...
                                      <p className="mt-1 text-[13px] font-semibold text-slate-900">
                                        {formatServiceMoney(
                                          service.currency,
                                          service.billableUnitRate || service.rate,
                                        )}
                                      </p>
                                    </div>
                                  </div>
                                  ...
                                  <div className="rounded-xl border border-blue-150 border-l-4 border-l-sky-500 bg-gradient-to-br from-sky-50/90 via-white to-blue-50/30 px-3.5 py-3 shadow-sm transition-all duration-500 ease-in-out hover:shadow-md hover:scale-[1.002]">
                                    <div className="flex items-start justify-between gap-3 xl:flex-col xl:items-start">
                                      <div>
                                        ...
                                        {Array.isArray(service.addonBreakdown) &&
                                          service.addonBreakdown.length > 0 ? (
                                          <div className="mt-2 space-y-1.5 rounded-lg border border-blue-100/80 bg-white/70 px-2 py-2">
                                            <p className="text-[9px] font-semibold uppercase tracking-[0.14em] text-blue-700/80">
                                              Added Extras
                                            </p>
                                            {service.addonBreakdown.map((addon) => (
                                              <div
                                                key={`${service.referenceServiceKey}-${addon.code}`}
                                              ...
                                              >
                                                <div className="min-w-0">
                                                  ...
                                                  <p className="text-slate-500">
                                                    {formatServiceMoney(service.currency, addon.rate)} x {addon.units}
                                                  </p>
                                                </div>
                                                <p className="shrink-0 font-semibold text-slate-900">
                                                  {formatServiceMoney(service.currency, addon.total)}
                                                </p>
                                              </div>
                                            ))}
                                          </div>
                              ...
                              >
                                ...
                                {referenceServices.map((reference) => {
                                  const isSelected = services.some(
                                    (s) => s.referenceServiceKey === reference.referenceServiceKey
                                  );
                                  return (
                                    <option
                                    ...
                                    >
                                      {isSelected ? "✓ " : ""}{reference.serviceName} | {serviceTypeLabel(reference.type)} | {formatServiceDate(reference.resolvedServiceDate)}
                                    </option>
                                  );
                                })}
                              </select>
                ...
                <div className="flex items-center gap-3">
                  <span className="hidden sm:inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em]"
                    style={{
                      ...
                      border: `1px solid ${travelerDocumentVerification.status === "Verified" ? "#bbf7d0" :
