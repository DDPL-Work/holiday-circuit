...
{/*=================================== Select Contracted Rates Service =============================== */}
            ...
            <motion.div variants={sectionRevealVariants} className="dark-scrollbar h-120 overflow-y-auto bg-black pr-1">
              <div className="sticky top-0 z-10 mb-3 flex items-center justify-between gap-3 bg-black p-2">
                <div>
...
{/* ==================================== Price Breakdown Section ============================================ */}
            ...
            <motion.div variants={rightCardVariants} className="bg-[#0e0e0e] border border-gray-700 rounded-xl p-4 text-sm space-y-4">
              ...
              <span  className={`${
                  servicesTotal > 0 ? "text-sky-500" : "text-red-400"}`}>INR {formatAmountValue(servicesTotal)}</span>
             </p>
              <p className="flex justify-between">
...
const Service = ({
  ...
  const sectionCardCls =
    "rounded-xl border border-[#1f1f1f] bg-[#101010] px-3 py-3";
  ...
  const calculateTripDayCountFromDate = (startDate, endDate) => {
    ...
    const diff = parsedEnd - parsedStart;
    return Math.max(1, Math.ceil(diff / (1000 * 60 * 60 * 24)));
  };
    ...
    >
      ...
      <div className="flex flex-col">
        ...
        <div className="flex-1 min-w-0">
          <div
            className={`relative border-b border-[#1a1a1a] p-4 pb-3 ${
              service.checked ? "pr-[210px]" : ""
            }`}
          >
          ...
          {service.checked && (
            <div className="absolute right-0 top-0 flex h-full min-w-[180px] flex-col justify-between border-l border-[#1a1a1a] border-b border-[#1a1a1a] px-4 py-4">
              <div className="text-right">
                <p className="mb-1 text-[9px] uppercase tracking-widest text-gray-300">Total</p>
              ...
              {service.custom && (
                <button
                  ...
                  className="mt-3 self-end rounded-lg p-1.5 text-gray-600 transition-all duration-150 hover:bg-red-400/10 hover:text-red-400"
                >
          ...
          <div className="space-y-3 px-4 pb-4">
            ...
            <div className="grid gap-3 lg:grid-cols-[minmax(0,220px)_minmax(0,180px)] lg:items-start">
              <div className={`${sectionCardCls} flex items-center justify-between gap-3`}>
                <div>
                  <p className={sectionLabelCls}>Base Rate</p>
              ...
              {isForeignCurrency && (
                <div className={`${sectionCardCls} flex h-full items-center justify-center`}>
                  <p className="text-[11px] text-slate-300">
                    1 {currencyCode} = <span className="text-sky-300">INR {formatExchangeRateValue(exchangeRate)}</span>
                  </p>
                </div>
              )}
            </div>
            ...
            {service.checked && (
              <div className={`${sectionCardCls} max-w-[340px]`}>
                <p className={sectionLabelCls}>Date</p>
                <input
                  type="date"
                  value={service.serviceDate || ""}
                  onChange={(e) => updateField(service.id, "serviceDate", e.target.value)}
                  className="mt-2 w-full bg-[#111111] border border-[#343434] hover:border-yellow-600/60 text-white text-[11px] rounded-lg px-3 py-2.5 pr-9 outline-none transition-colors duration-150 focus:border-yellow-500 [color-scheme:dark] [&::-webkit-calendar-picker-indicator]:cursor-pointer [&::-webkit-calendar-picker-indicator]:invert [&::-webkit-calendar-picker-indicator]:brightness-[3] [&::-webkit-calendar-picker-indicator]:opacity-100 [&::-webkit-calendar-picker-indicator]:drop-shadow-[0_0_2px_rgba(255,255,255,0.9)]"
                />
              </div>
            )}
            ...
            {service.checked && service.type === "hotel" && (
              <div className={`${sectionCardCls} space-y-3`}>
                <div className="flex flex-wrap gap-x-4 gap-y-2">
                  ...
                  {tripNights > 0 && hotelNightStart > 0 && (
                    <span className="text-[10px] text-yellow-300">
                      ...
                      {remainingHotelNights > 1 ? `-${Math.min(tripNights, hotelNightStart + remainingHotelNights - 1)}` : ""}
                    </span>
                  )}
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <span className={`${sectionLabelCls} text-gray-300`}>Nights</span>
                  <select
                    value={service.nights || ""}
                    onChange={(e) => updateField(service.id, "nights", e.target.value)}
                    className={`${selectCls} min-w-[210px]`}
                  >
            ...
            {service.checked && (service.type === "transfer" || service.type === "car") && (
              <div className={`${sectionCardCls} space-y-3`}>
                <div className="flex flex-wrap gap-x-4 gap-y-2">
                  {transportStartDate && (
                    <span className="text-[10px] text-emerald-400">
                      Covers up to {maxTransportDays} trip day{maxTransportDays > 1 ? "s" : ""} from this start date
                    </span>
                  )}
                  {transportStartDate && transportEndDate && (
                    <span className="text-[10px] text-sky-300">
                      Coverage: {transportStartDate === transportEndDate
                        ? formatDisplayDate(transportStartDate)
                        : `${formatDisplayDate(transportStartDate)} to ${formatDisplayDate(transportEndDate)}`}
                    </span>
                  )}
                </div>

                <div className="flex flex-wrap items-center gap-3">
                  <span className={`${sectionLabelCls} text-gray-300`}>Service</span>
                  <select
                  ...
                  <span className={`${sectionLabelCls} text-gray-300`}>Days</span>
                  <select
                  ...
                  >
                    {[...Array(maxTransportDays)].map((_, i) => (
                      <option key={i} value={i + 1}>
                        ...
                        {transportStartDate
                          ? ` (${formatDisplayDate(transportStartDate)}${
                              i > 0 ? ` to ${formatDisplayDate(addDaysToServiceDate(transportStartDate, i))}` : ""
                            })`
                          : ""}
                      </option>
                    ))}
            ...
            {service.checked && service.type === "activity" && (
              <div className={`${sectionCardCls} flex items-center gap-3`}>
                <span className={`${sectionLabelCls} text-gray-300`}>PAX</span>
                <input
                  type="number"
                  value={service.pax}
            ...
            {service.checked && service.type === "sightseeing" && (
              <div className={`${sectionCardCls} flex flex-wrap items-center gap-3`}>
                <span className={`${sectionLabelCls} text-gray-300`}>PAX</span>
                <input
                  type="number"
                  value={service.pax}
          ...
          {service.checked && service.type === "hotel" && (
            <div className="mt-2.5 grid gap-2 pl-[52px] sm:max-w-[520px]">
              {/* A.W.E.B */}
              <label className="grid grid-cols-[auto_1fr] items-center gap-3 rounded-lg border border-[#222] bg-[#141414] px-3 py-2 cursor-pointer transition-colors duration-150 hover:border-yellow-500/30">
                <input
                  type="checkbox"
                ...
                <div className="flex items-center justify-between gap-3">
                  <span className="text-[10px] text-gray-400">A.W.E.B</span>
                  <div className="flex flex-wrap items-center justify-end gap-x-2 gap-y-1 text-[10px]">
                    <span className="font-medium text-yellow-400">
                      {formatCurrencyValue(service.awebRate || 0, currencyCode)}
                    </span>
                    {isForeignCurrency && (
                      <span className="text-sky-300">
                        INR {formatAmountValue(convertAmountToInr(service.awebRate || 0, currencyCode, exchangeRates))}
                      </span>
                    )}
                  </div>
                </div>
              ...
              {/* C.W.E.B */}
              <label className="grid grid-cols-[auto_1fr] items-center gap-3 rounded-lg border border-[#222] bg-[#141414] px-3 py-2 cursor-pointer transition-colors duration-150 hover:border-emerald-500/30">
                <input
                  type="checkbox"
                ...
                />
                <div className="flex items-center justify-between gap-3">
                  <span className="text-[10px] text-gray-400">C.W.E.B</span>
                  <div className="flex flex-wrap items-center justify-end gap-x-2 gap-y-1 text-[10px]">
                    <span className="font-medium text-emerald-400">
                      {formatCurrencyValue(service.cwebRate || 0, currencyCode)}
                    </span>
                    {isForeignCurrency && (
                      <span className="text-sky-300">
                        INR {formatAmountValue(convertAmountToInr(service.cwebRate || 0, currencyCode, exchangeRates))}
                      </span>
                    )}
                  </div>
                </div>
              ...
              {/* C.Wo.E.B */}
              <label className="grid grid-cols-[auto_1fr] items-center gap-3 rounded-lg border border-[#222] bg-[#141414] px-3 py-2 cursor-pointer transition-colors duration-150 hover:border-blue-500/30">
                <input
                  type="checkbox"
                ...
                />
                <div className="flex items-center justify-between gap-3">
                  <span className="text-[10px] text-gray-400">C.Wo.E.B</span>
                  <div className="flex flex-wrap items-center justify-end gap-x-2 gap-y-1 text-[10px]">
                    <span className="font-medium text-blue-400">
                      {formatCurrencyValue(service.cwoebRate || 0, currencyCode)}
                    </span>
                    {isForeignCurrency && (
                      <span className="text-sky-300">
                        INR {formatAmountValue(convertAmountToInr(service.cwoebRate || 0, currencyCode, exchangeRates))}
                      </span>
                    )}
                  </div>
                </div>
